...
async def game(ctx, rounds: int = 1):
    channel = ctx.channel
    ...
    async def run_round(round_number):
        ...
        embed = discord.Embed(title=f"Guess what station this photo was taken at!", color=0xd8d500, description="Type ! before your answer. You have 30 Seconds")
        if rounds != 1:
            embed.set_author(name=f"Round {round_number + 1} of {rounds}", url='https://xm9g.xyz')
