...
try:    
    type = int(input("What type of Public Transport? Choose 0 for Metro, 1 for Tram, 2 for Bus, 3 for V/Line"))
except:
    print("Try again, make sure to only input a number")
...
if type == 3:
    ...
    print("Try again, make sure to only input a number from 0 - 3")
...
def api_route_list(number):
    try:
        # API endpoint URL
        url = getUrl(f'/v3/routes?route_types={number}')
...
def api_stop_list(number, route_id):
    try:
        # API endpoint URL
        url = getUrl(f'/v3/stops/route/{route_id}/route_type/{number}')
        print(f"Search url: {url}")
...
pre_file = open(f'utils\\datalists\\{type_name}stops.txt','w')
pre_file.write('')
file = open(f'utils\\datalists\\{type_name}stops.txt','a')
...
file = open(f'utils\\datalists\\{type_name}stops.txt','r')
lines = file.readlines()
file = open(f'utils\\datalists\\{type_name}stops.txt','w')
